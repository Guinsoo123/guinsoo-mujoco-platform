...
- 算法示例：经典关节位置控制，以及 UR5e 末端避障到点（RRT 规划 + 路径跟踪）等独立 Demo。
...
Sim Studio 使用统一 Python logging，日志同时输出到：

- 中间面板 **实时曲线下方** 的运行日志区（UI 实时滚动）
- 终端 stderr（INFO 及以上）
- 本地文件 `~/.guinsoo_mujoco/logs/sim_studio_YYYYMMDD-HHMMSS.log`

日志来源包括：UI 操作、场景加载、MuJoCo 警告、控制器状态切换、RRT/IK 规划结果。

如需更详细的步进级日志，可设置环境变量：

```bash
GUINSOO_LOG_LEVEL=DEBUG guinsoo-sim-studio
```

CLI 命令日志写入 `~/.guinsoo_mujoco/logs/cli_*.log`。
...
每次录制会保存到独立子目录，命名格式为 `时间-demo名称`，例如：

```bash
~/.guinsoo_mujoco/runs/20260610-155550-ee_pose_avoid/
```
...
- `src/guinsoo_mujoco/demos/`：独立 Demo 包（场景、规划、文档）。
- `src/guinsoo_mujoco/logging_config.py`：统一日志配置与 MuJoCo 警告接入。
