...
### 2. 用 PlotJuggler 画图分析

1. 安装 [PlotJuggler](https://github.com/facontidavide/PlotJuggler)。
2. 打开 PlotJuggler，选择 **Data Load** → **CSV**。
3. 载入 `*_plotjuggler.csv`，将 `timestamp` 设为横轴。
4. 在左侧信号树中选择例如 `joint/shoulder_pan/qpos`、`joint/shoulder_pan/target`、`joint/shoulder_pan/actuator_force` 进行对比。
