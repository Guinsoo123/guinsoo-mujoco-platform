...
from guinsoo_mujoco.logging_config import get_logger, setup_logging
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command == "fetch-assets":
        ...
        if manifest.license == "UNKNOWN":
            logger.error(
                "无法下载 %s：资产许可证为 UNKNOWN，请先完成来源和许可证复核。",
                robot.robot_id,
            )
            return 2
        cache_path = AssetDownloader(args.cache_root).fetch(manifest)
        logger.info("资产已缓存到: %s", cache_path)
        return 0
    ...
    if args.command == "export-plotjuggler":
        ...
        logger.info("PlotJuggler CSV 已导出到: %s", csv_path)
        return 0
