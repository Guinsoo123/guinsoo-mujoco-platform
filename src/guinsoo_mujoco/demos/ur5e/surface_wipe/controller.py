...
from guinsoo_mujoco.operators.ik import IkOptions, solve_ik_multi_seed, solve_ik_nearest
from guinsoo_mujoco.operators.path import actuator_joint_target, unwrap_joint_target
...
class SurfaceWipeController:
    ...
    def _step_approach(self, runtime: MuJoCoRuntime, dt: float) -> dict:
        ...
        if float(np.linalg.norm(pos - target_pose[0])) < APPROACH_REACH_TOL:
            q_target, _ = runtime.read_joint_state()
            self._apply_joint_target(runtime, q_target)
    ...
    def _step_descend(self, runtime: MuJoCoRuntime, dt: float) -> dict:
        ...
        if (
            self.descend_elapsed >= MIN_DESCEND_TIME
            and tool_contact
            and CONTACT_FORCE_THRESHOLD <= force_normal <= MAX_CONTACT_FORCE
        ):
    ...
    def _step_follow(self, runtime: MuJoCoRuntime, dt: float) -> dict:
        ...
        normal_offset = CONTACT_STANDOFF + d_n
        target_pose = self._desired_pose(
            s=self.path_s,
    ...
    def _read_normal_force(self, runtime: MuJoCoRuntime, *, s: float) -> float:
        ...
        n_hat = SURFACE.normal(s)
        force_normal = project_force_on_normal(force_world, n_hat)
        self.last_force_normal = float(
            np.clip(force_normal, 0.0, MAX_CONTACT_FORCE)
        )
        return self.last_force_normal
    ...
    def _solve_pose(
        ...
        *,
        ...
        fallback = (PREP_QPOS, WIPE_IK_SEED)
        if prefer_nearest:
            q = solve_ik_nearest(
                runtime,
                pos,
    ...
    def _apply_joint_target(self, runtime: MuJoCoRuntime, q_target: np.ndarray) -> None:
        qpos, _ = runtime.read_joint_state()
    ...
    def _sample(self, runtime: MuJoCoRuntime, q_target: np.ndarray) -> dict:
        ...
        signed_standoff = float(np.dot(pos - p_ref, n_hat))
        return {
